...
inline void log_complex_array(char *filename, fftw_complex *array, int32_t len) {

	int i;	
	FILE *file;
	file = fopen(filename,"w+"); // clear file if exists
	
	log_int32_t("Saving complex array to file - array length", len);
	for ( i = 0; i < len; i++ )
	{
		fprintf (file, "%4d,%12f,%12f\n", i+1, array[i][0], array[i][1] );
	}
	fclose(file); 
}
...
int32_t gettimeold()	// in milliseconds
{
