...
namespace Wave2ZebraSynth
{
	...
	{				
        ...
        /// <param name = "fftWindowsSize">Length of the window</param>
        /// <returns>Window function</returns>		
		public static void ApplyWindowFunction(int whichFunction, int fftWindowsSize, double[] @in)
		{
			int i;
			double A;
			switch(whichFunction)
			{
				...
				case 1:
					...
					{
						@in[i] *= (i / (fftWindowsSize / 2));
						@in[i + (fftWindowsSize / 2)] *= (1.0f - (i / (fftWindowsSize / 2)));
					}
					break;
				case 2:
					// Hamming
					for (i = 0; i < fftWindowsSize; i++)
						@in[i] *= (0.54 - 0.46 * Math.Cos(2 * Math.PI * i / (fftWindowsSize - 1)));
					break;
				case 3:
					// Hanning
					for (i = 0; i < fftWindowsSize; i++)
						@in[i] *= (0.50 - 0.50 * Math.Cos(2 * Math.PI * i / (fftWindowsSize - 1)));
					break;
				case 4:
					// Blackman
					for (i = 0; i < fftWindowsSize; i++)
					{
						@in[i] *= (0.42 - 0.5 * Math.Cos (2 * Math.PI * i / (fftWindowsSize - 1)) + 0.08 * Math.Cos (4 * Math.PI * i / (fftWindowsSize - 1)));
					}
					break;
				case 5:
					// Blackman-Harris
					for (i = 0; i < fftWindowsSize; i++)
					{
						@in[i] *= (0.35875 - 0.48829 * Math.Cos(2 * Math.PI * i /(fftWindowsSize-1)) + 0.14128 * Math.Cos(4 * Math.PI * i/(fftWindowsSize-1)) - 0.01168 * Math.Cos(6 * Math.PI * i/(fftWindowsSize-1)));
					}
					break;
				case 6:
					// Welch
					for (i = 0; i < fftWindowsSize; i++)
					{
						@in[i] *= 4 *i/fftWindowsSize*(1-(i/fftWindowsSize));
					}
					break;
				case 7:
					// Gaussian (a=2.5)
					...
					for (i = 0; i < fftWindowsSize; i++)
					{
						...
						// reduced
						@in[i] *= Math.Exp(A*(0.25 + ((i/fftWindowsSize)*(i/fftWindowsSize)) - (i/fftWindowsSize)));
					}
					break;
				case 8:
					// Gaussian (a=3.5)
