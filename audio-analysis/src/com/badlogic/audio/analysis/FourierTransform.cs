...
namespace com.badlogic.audio.analysis
{
	...
	public abstract class FourierTransform
	{
		...
		protected internal const int LINAVG = 2;
		protected internal const int LOGAVG = 3;
		protected internal const int NOAVG = 4;
		protected internal const float TWO_PI = (float)(2 * Math.PI);
		protected internal int timeSize;
		protected internal int sampleRate;
		protected internal float bandWidth;
		protected internal int whichWindow;
		protected internal float[] real;
		protected internal float[] imag;
		protected internal float[] spectrum;
		protected internal float[] averages;
		protected internal int whichAverage;
		protected internal int octaves;
		protected internal int avgPerOctave;
		...
		//
		internal FourierTransform(int ts, float sr)
		{
			timeSize = ts;
			sampleRate = (int)sr;
		...
		protected internal abstract void AllocateArrays();

		protected internal void SetComplex(float[] r, float[] i)
		{
			...
			{
				System.Array.Copy(r, 0, real, 0, r.Length);
				System.Array.Copy(i, 0, imag, 0, i.Length);
			}
		}
		...
		protected internal void FillSpectrum()
		{
		...
		protected internal void DoWindow(float[] samples)
		{
			switch (whichWindow)
			{
				case HAMMING:
		...
		protected internal void Hamming(float[] samples)
		{
		...
		public void Forward(float[] buffer, int startAt)
		{
			...
			System.Array.Copy(buffer, startAt, section, 0, section.Length);
			Forward(section);
		}
